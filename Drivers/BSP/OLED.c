...
{	
	...
	GPIO_InitStructure.Pull = GPIO_NOPULL;					// 默认浮空
...
void OLED_ShowImg(uint8_t Line, uint8_t Column, uint8_t islock)
{
	...
	{		
		if(islock)
		{
			OLED_WriteData(OLED_F16x16_islock[islock][i]);
		}
		else 
			OLED_WriteData(OLED_F16x16_islock[islock][i]);
	}
	...
	for(i = 0; i<16; i++)
	{
		if(islock)
		{
			OLED_WriteData(OLED_F16x16_islock[islock][i + 16]);
		}
		else
			OLED_WriteData(OLED_F16x16_islock[islock][i + 16]);
	}
...
void OLED_ShowChinese_TimeWord(uint8_t Line, uint8_t Column, uint8_t N)
{
	...
	for(i = 0; i<16; i++)
	{		
		OLED_WriteData(OLED_F16x16_timeWord[N][i]);
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);		// 显示下半部分
	for(i = 0; i<16; i++)
	{			
		OLED_WriteData(OLED_F16x16_timeWord[N][i + 16]);
	}
}
...
void OLED_ShowChineseTips(uint8_t Line, uint8_t Column, uint8_t N)
{
	...
	for(i = 0; i<16; i++)
	{		
		OLED_WriteData(OLED_F16x16_tipWord[N][i]);
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);		// 显示下半部分
	for(i = 0; i<16; i++)
	{			
		OLED_WriteData(OLED_F16x16_tipWord[N][i + 16]);
	}
}
